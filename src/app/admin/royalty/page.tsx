...
import { Percent, DollarSign, Loader2, TrendingUp, Calendar } from "lucide-react";
...
export default function RoyaltyReports() {
        ...
        logs.forEach((log: any) => {
          ...
          if (existing) {
            ...
            cafeMap.set(log.cafe_id, {
              cafe_name: name,
              royalty_pct: Number(log.cafes?.royalty_percentage || log.royalty_percentage),
              total_orders: 1,
          ...
          daysMap[d] = (daysMap[d] || 0) + Number(log.royalty_amount);
        });

        const chart = Object.entries(daysMap)
          .map(([date, royalty]) => ({ date, royalty: Math.round(royalty) }))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <GlassCard>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Total Royalty Earned</p>
        ...
        <GlassCard>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Active Cafes</p>
      ...
      <GlassCard>
        <h3 className="text-lg font-semibold mb-4">Cafe-wise Royalty</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {summary.map((s, i) => (
                ...
                >
                  <td className="py-3 px-4 font-medium">{s.cafe_name}</td>
                  <td className="py-3 px-4 text-right">
                    <Badge variant="secondary">{s.royalty_pct}%</Badge>
                  </td>
                  <td className="py-3 px-4 text-right">{s.total_orders}</td>
                  <td className="py-3 px-4 text-right">
