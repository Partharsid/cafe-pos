...
export default function SuperAdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      if (cafes && orders) {
        const cafeMap = new Map<string, { name: string; orders: number; revenue: number; royalty: number }>();
        cafes.forEach((c) =>
          cafeMap.set(c.id, { name: c.name, orders: 0, revenue: 0, royalty: 0 })
        );
        ...
        setStats({
          ...
          totalRoyalty: orders.reduce((s, o) => s + Number(o.royalty_amount), 0),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <GlassCard>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Royalty Earned</p>
              <p className="text-2xl font-bold">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={stats?.revenueByDay}>
              ...
              />
              <Bar dataKey="revenue" fill="oklch(0.66 0.19 258.5)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </GlassCard>
      ...
      <GlassCard>
        <h3 className="text-lg font-semibold mb-4">Cafe-wise Breakdown</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {stats?.cafeStats.map((cs) => (
                <tr key={cs.cafe_id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  <td className="py-3 px-4 font-medium">{cs.cafe_name}</td>
                  <td className="py-3 px-4 text-right">{cs.total_orders}</td>
